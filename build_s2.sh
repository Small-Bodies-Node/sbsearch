...
if [ ! -e ${PREFIX}/lib/libabsl_base.a ]; then
    ...
    mkdir -p build
    cd build
    cmake -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_CXX_STANDARD=${CXX_STANDARD} -DCMAKE_INSTALL_PREFIX=${PREFIX} -DABSL_ENABLE_INSTALL=ON -DABSL_PROPAGATE_CXX_STD=ON ..
    make -j $(nproc)
    make install
    cd ../..
fi
...
if [ ! -e ${PREFIX}/lib/libs2.so ]; then
    ...
    mkdir -p build
    cd build
    cmake -DGOOGLETEST_ROOT=/usr/src/googletest -DWITH_PYTHON=ON -DCMAKE_PREFIX_PATH=${PREFIX} -DCMAKE_CXX_STANDARD=${CXX_STANDARD} -DCMAKE_INSTALL_PREFIX=${PREFIX} -Wno-dev -DPython3_FIND_STRATEGY=LOCATION -DPython3_ROOT_DIR=${PYTHON_ROOT} ..
    make -j $(nproc)
    make install
fi
