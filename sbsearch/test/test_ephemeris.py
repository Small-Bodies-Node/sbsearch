...
def encke() -> List[Ephemeris]:
    ...
    return g.target_over_date_range("500@", target, start, stop, step=step, cache=True)
...
class TestHorizons:
    ...
    def test_at_dates(self):
        ...
        g: Horizons = Horizons
        eph: List[Ephemeris] = g.target_at_dates("500@", target, dates, cache=True)
        assert np.allclose([e.ra for e in eph], [120.97483, 65.4021], rtol=1e-3)
        assert np.allclose([e.dec for e in eph], [17.72957, 26.36761], rtol=1e-3)
    ...
    def test_at_dates_single(self):
        ...
        g: Horizons = Horizons
        eph: List[Ephemeris] = g.target_at_dates("500@", target, date, cache=True)
    ...
    def test_over_date_range(self, encke):
        assert np.allclose(
            [e.ra for e in encke], [65.4021, 81.36382, 100.97247, 120.97483], rtol=1e-3
        )
        assert np.allclose(
            [e.dec for e in encke], [26.36761, 26.87021, 24.42504, 17.72957], rtol=1e-3
        )
        assert np.allclose([e.mjd for e in encke], [59001.0, 59011.0, 59021.0, 59031.0])

    @pytest.mark.parametrize(
        "start,stop", (("2018-08-13", "2018-08-17"), ("2018-11-03", "2018-11-07"))
    )
    def test_over_date_range_adaptable(self, start, stop):
        ...
        stop: Time = Time(stop)
        eph: List[Ephemeris] = g.target_over_date_range(
