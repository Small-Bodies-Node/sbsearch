...
from ..exceptions import (DatabaseNotConnected, PrimaryDesignationError,
...
class TestFixedTarget:
    ...
    def test_ephemeris_at_dates(self):
        ...
        eph: List[Ephemeris] = (FixedTarget(SkyCoord(123 * u.deg, 45.6 * u.deg))
...
class TestMovingTarget:
    ...
    def test_ephemeris_at_dates(self):
        ...
        assert np.allclose([e.ra for e in eph], [
                           65.4021, 120.97483], rtol=1e-3)
        assert np.allclose([e.dec for e in eph], [
                           26.36761, 17.72957], rtol=1e-3)
    ...
    def test_ephemeris_over_date_range(self):
        ...
        assert np.allclose([e.dec for e in eph],
                           ...
                           rtol=1e-3)
        assert np.allclose([e.mjd for e in eph],
