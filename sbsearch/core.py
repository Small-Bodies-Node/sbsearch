...
# limits on segment lengths generated by line_to_segment_query_terms
ARC_LIMIT: float = 0.17  # radians
TIME_LIMIT: float = 365  # days
...
def line_to_segment_query_terms(indexer: SpatialIndexer,
    ...
    Query every ``ARC_LIMIT`` of motion or ``TIME_LIMIT`` of observations,
    linear estimates are OK for this.  10 deg / 30 days = 50 arcsec/hr.
    ...
    while True:
        ...
        if query_about:
            ...
            terms = indexer.query_line(ra[i:j + 1],
                                       dec[i:j + 1])

        yield terms, slice(i, j + 1)
