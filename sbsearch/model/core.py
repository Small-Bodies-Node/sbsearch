...
class Designation(Base):
    ...
    object_id: int = Column(
        Integer,
        ForeignKey("obj.object_id", onupdate="CASCADE", ondelete="CASCADE"),
        index=True,
        nullable=False,
    )
...
class Ephemeris(Base):
    ...
    mjd: float = Column(
        Float(32), index=True, nullable=False, doc="Modified Julian date, UT"
    )
    ...
    ra: float = Column(Float(32), nullable=False, doc="Right Ascension ICRF, deg")
    dec: float = Column(Float(32), nullable=False, doc="Declination ICRF, deg")
    dra: float = Column(
        Float(32), doc=("sky motion, includes cos(Dec) factor," " arcsec/hr")
    )
    ddec: float = Column(Float(32), doc="arcsec/hr")
    unc_a: float = Column(Float(32), doc="error ellipse semi-major axis, arcsec")
    unc_b: float = Column(Float(32), doc="error ellipse semi-minor axis, arcsec")
    ...
    sangle: float = Column(Float(32), doc="projected comet-Sun position angle, deg")
    vangle: float = Column(
...
class Observation(Base):
    ...
    source: str = Column(String(64), default="observation", doc="source survey")
    mjd_start: float = Column(
        Float(32),
        nullable=False,
        index=True,
        doc="shutter open, modified Julian date, UTC",
    )
    mjd_stop: float = Column(
        Float(32),
        nullable=False,
        index=True,
    ...
    spatial_terms: List[str] = Column(
        ARRAY(Text), nullable=False, doc="spatial index terms"
    )
    ...
    __mapper_args__ = {"polymorphic_identity": "observation", "polymorphic_on": source}
...
class Found(Base):
    ...
    orbit_id = Column(
        Integer,
        ForeignKey("orbit.orbit_id", onupdate="CASCADE", ondelete="CASCADE"),
        index=True,
    )
    ...
    mjd: float = Column(
        Float(32), index=True, nullable=False, doc="Modified Julian date, UTC"
    )
    rh: float = Column(Float(32), doc="heliocentric distance, au")
    delta: float = Column(Float(32), doc="observer-target distance, au")
