...
namespace sbsearch
{
    namespace horizons
    {
        const string format_command(const string target, const double mjd, const bool small_body)
        {
            ...
            bool temporary_comet = (target.find_first_of("CPDI") == 0) & (target[1] == '/');
            ...
            if (!temporary_comet)
            {
                size_t letter = target.find_first_of("DPI");
            ...
            if (comet)
            {
                if (mjd > 0)
                {
                    ...
                    return "DES=" + target + ";NOFRAG;CAP<" + std::to_string(jd) + ";";
                }
                else
                    return "DES=" + target + ";NOFRAG;CAP;";
            }

            if (target.find_first_of("0123456789") == string::npos)
...
OBJ_DATA='YES'
        ...
        const string query(const string parameters, const bool cache)
        {
